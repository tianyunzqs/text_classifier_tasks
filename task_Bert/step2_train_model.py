...
tf.flags.DEFINE_string("config_file", "./config_file", "config file for model to predict online.")
...
class NewsProcessor(DataProcessor):
    ...
    def _read_tsv(cls, input_file, quotechar=None):
        with open(input_file, "r", encoding="utf-8") as f:
            lines = f.readlines()
        random.shuffle(lines)  # 注意要shuffle
        return lines
...
def config_model(label_list):
    config = collections.OrderedDict()

    config["max_seq_length"] = FLAGS.max_seq_length
...
def main(_):
    ...
    config = config_model(label_list=label_list)
