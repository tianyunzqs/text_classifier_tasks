...
class Evaluation(object):
    ...
    def load_model(self):
        ...
        with tf.Graph().as_default():
            ...
            with self.sess.as_default():
                self.transformer = TextTransformer(vocab_size=self.vocab.size(),
                                                   num_embedding_size=self.config['embedding_size'],
                                                   filters=self.config['filters'],
                                                   num_classes=len(self.config['labels']),
                                                   sequence_length=self.config['num_timesteps'],
    ...
    def evaluate_line(self, text):
        ...
        predictions = self.sess.run(
            [self.transformer.predictions],
            feed_dict={
                self.transformer.inputs: [id_words] * self.config['batch_size'],
                self.transformer.keep_prob: 1.0
            })
