...
from task_Transformer.data_helper import Vocab, CategeoryDict, TextDataSet, compute_p_r_f
from task_Transformer.textTransformer import TextTransformer
...
tf.flags.DEFINE_integer("representation_output_size", 128, "经过RNN后表示层的输出大小")
tf.flags.DEFINE_integer("filters", 128, "卷积核的数量")
tf.flags.DEFINE_integer("num_blocks", 1, "transformer层数")
tf.flags.DEFINE_integer("num_heads", 8, "multi head 的头数")
tf.flags.DEFINE_float("ln_epsilon", 1e-8, "")
...
def save_config(vocab, categeory):
    ...
    config["representation_output_size"] = FLAGS.representation_output_size
    config["filters"] = FLAGS.filters
    config["num_blocks"] = FLAGS.num_blocks
    config["num_heads"] = FLAGS.num_heads
    config["ln_epsilon"] = FLAGS.ln_epsilon
...
transformer = TextTransformer(vocab_size=vocab_size,
                              num_embedding_size=FLAGS.embedding_size,
                              filters=FLAGS.filters,
                              num_classes=num_classes,
                              sequence_length=FLAGS.num_timesteps,
                              num_blocks=FLAGS.num_blocks,
                              batch_size=FLAGS.batch_size,
                              num_heads=FLAGS.num_heads,
                              epsilon=FLAGS.ln_epsilon)


# Define Training procedure
global_step = tf.Variable(0, name="global_step", trainable=False)
optimizer = tf.train.AdamOptimizer(1e-3)
grads_and_vars = optimizer.compute_gradients(transformer.loss)
...
with tf.Session() as sess:
    ...
    for epoch, batch_inputs, batch_labels in train_dataset.batch_iter(FLAGS.batch_size, FLAGS.num_epochs):
        _, loss_val, accuracy_val, current_step = sess.run(
            [train_op, transformer.loss, transformer.accuracy, global_step],
            feed_dict={
                transformer.inputs: batch_inputs,
                transformer.outputs: batch_labels,
                transformer.keep_prob: 0.8
            })
        ...
        if current_step % FLAGS.evaluate_every == 0:
            ...
            for _, val_batch_inputs, val_batch_labels in val_dataset.batch_iter(FLAGS.batch_size, 1):
                _, val_predictions = sess.run(
                    [train_op, transformer.predictions],
                    feed_dict={
                        transformer.inputs: val_batch_inputs,
                        transformer.outputs: val_batch_labels,
                        transformer.keep_prob: 1.0
                    })
            ...
            for _, test_batch_inputs, test_batch_labels in test_dataset.batch_iter(FLAGS.batch_size, 1):
                _, test_predictions = sess.run(
                    [train_op, transformer.predictions],
                    feed_dict={
                        transformer.inputs: test_batch_inputs,
                        transformer.outputs: test_batch_labels,
                        transformer.keep_prob: 1.0
                    })
